...
{							//发送74字节应答包
	...
	if (temp_block == Caltab_block)
	{
		if (data74[3] == 0)
		{
			//Erase_DF321block(1022);
			//Delay_mS(5);
			Read_DF321page(1022,j,64,data74+9);
			//Delay_mS(5);
		}
		else if (data74[3] == 1)
		{
		...
		{
			//Erase_DF321block(1023);
			//Delay_mS(5);
			Read_DF321page(1023,j,64,data74+9);
			//Delay_mS(5);	
		}
		else if (data74[3] == 3)
		{
...
void Rxpack(uint temp_block)//接收数据包，发送8字节应答包
{
	...
	{//因为校对表长度为4096字节，为防止连续写FLASH出错，在写前后加一定延时
		...
		if (data74[3] == 0)
		{
			Erase_DF321block(1022);
			Delay_mS(5);
			Write_DF321page(1022,j,64,data74+9);
			Delay_mS(5);
		}
		else if (data74[3] == 1)
		{
		...
		else if (data74[3] == 2)
		{
			Erase_DF321block(1023);
			Delay_mS(5);
			Write_DF321page(1023,j,64,data74+9);
			Delay_mS(5);	
		}
		else if (data74[3] == 3)
		{
...
void Send_davrfDebug(uchar N)		//发送测试数据，长度为64
{					
	uchar i;
	//data74[N]=0;
	//Tx(0X55);
