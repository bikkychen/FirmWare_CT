...
{
	//uint b=0;
	ADCON1=0X3A;			//RA0为单片机内部AD，其余IO复用口全部设为IO口
...
//ZFCY--采集AIN1通道
void AD_10BIT1(void)			//4ms,内部单片机AD单次采集AN0
{
	//uint b=0;
	ADCON1=0X3A;			//RA0为单片机内部AD，其余IO复用口全部设为IO口
	ADCON0=0X04;			//选择A/D 输入通道AN1
	ADCON2=0XA1;            //数据左对齐21，选择A/D 采集时间8TAD,选择A/D 转换时钟001 = FOSC/8,101 = FOSC/16
	ADCON0bits.ADON=1;		//使能AD转换模块
	Delay_mS(4);            //2
	if(TX_8M)
...
{	
	...
	bn=BLOCKNUM;
	//if(para.info.mem_num<1 || para.info.mem_num>2 )//20170710
		para.info.mem_num=1;
	para.info.mem_num-=1;
...
void PreAD7799_P1(void)//通道1 ----14.59ms-----20130928
{	
	//TX=1;
	CS0
	Nop();Nop();Nop();Nop();
...
void ReadAD7799_16P1(void)//通道1 ----137.42ms-----20130928
{	
	//TX=1;
	WriteAD7799(0X08);//指定模式寄存器
	WriteAD7799(0X00);//单次转换模式0X20
	WriteAD7799(0X0A); //0X07转换频率33.3Hz,T_settle=60mS	
						//0X0A转换频率16.7Hz,T_settle=120mS
	WriteAD7799(0X10);//指定配置寄存器
	WriteAD7799(Pgain);//有极性，增益不定
	WriteAD7799(0X11);//缓冲，通道2 
	
	Delay_mS(130); //(0X0A)留出15mS的余量-120ms-----20130129
	//Delay_mS(70); 
	if(TX_8M)
		Delay_mS(130);
		//Delay_mS(70);
...
void ReadAD7799_16P2(void)  //通道2----82ms-----20130129
{
	//TX=1;	
	WriteAD7799(0X08);//指定模式寄存器
	WriteAD7799(0X00);//单次转换模式0X20
	WriteAD7799(0X0A);//(0X07)转换频率33.3Hz,T_settle=60mS

	WriteAD7799(0X10);//指定配置寄存器
	WriteAD7799(Pgain);//有极性，增益不定
	WriteAD7799(0X11);//缓冲，通道2 
	Delay_mS(130);     // 留出15mS的余量	
	if(TX_8M)
		Delay_mS(130);
	WriteAD7799(0X58);//读AD转换结果
	value.l=0;	
	...
	value.ch4[3]=0;
	value.l=(value.l)>>8;
	value.l+=para.info.adjzero.l;  

}

void ReadAD7799_16T1(void)  //通道2 ----77.45ms-----20130929
{	
	//TX=1;
	WriteAD7799(0X08);//指定模式寄存器
	WriteAD7799(0X00);//单次转换模式0X20
	...
	WriteAD7799(0X10);//指定配置寄存器
	WriteAD7799(0X10);//无极性，增益1
	WriteAD7799(0X02);//无缓冲，通道3 
	
	Delay_mS(70);//留出10mS的余量	
	if(TX_8M)
		Delay_mS(70);
	WriteAD7799(0X58);//读AD转换结果
	value.l=0;	
...
{		
	...
	WriteAD7799(0X08);//指定模式寄存器
	WriteAD7799(0X00);//单次转换模式0X20
	WriteAD7799(0X07);//0X07转换频率33.3Hz,T_settle=60mS//0X0A转换频率16.7Hz,T_settle=120mS
	Delay_mS(70);//0X0C转换频率10Hz,T_settle=200mS留出15mS的余量	
	if(TX_8M)
		Delay_mS(70);
	WriteAD7799(0X58);//读AD转换结果
	value.l=0;	
...
void ReadAD7799_24P1(void)//通道1 ----150ms-----20130809
{	
	//TX=1;
	WriteAD7799(0X10);//指定配置寄存器
	WriteAD7799(Pgain);//有极性，增益不定
	...
	WriteAD7799(0X08);//指定模式寄存器
	WriteAD7799(0X00);//单次转换模式0X20
	WriteAD7799(0X0E); //转换频率6.25Hz,T_settle=320mS		
	Delay_mS(335);     //   留出20mS的余量
	if(TX_8M)
		Delay_mS(335);
	WriteAD7799(0X58);//读AD转换结果
	value.l=0;	
...
{	
	...
	WriteAD7799(0X08);//指定模式寄存器
	WriteAD7799(0X00);//单次转换模式0X20
	WriteAD7799(0X0A);//0X0E转换频率6.25Hz,T_settle=320mS//0X07转换频率33.3Hz,T_settle=60mS	
	Delay_mS(130);    //340ms留出20mS的余量//0X0B转换频率12.5Hz,T_settle=160mS		
	if(TX_8M)
		Delay_mS(130);
	WriteAD7799(0X58);//读AD转换结果
	value.l=0;	
...
void ReadAD_MODE1(void)  //---362ms---20140514
{
	CS0	
	ReadAD7799_16T2();
	data74[4]=value.ch4[1];
	data74[5]=value.ch4[2];
	Delay_mS(5);
	if(TX_8M)
		Delay_mS(5);				
	ReadAD7799_16P2();
...
void ReadAD_MODE2(void)  //---302ms---20140514
{
	CS0	
	ReadAD7799_16T2();
	data74[4]=value.ch4[1];
...
void ReadAD_MODE6(void)  //---484ms---20140514
{
	CS0					
	ReadAD7799_24T1();
	data74[3]=value.ch4[0];
	data74[4]=value.ch4[1];
	data74[5]=value.ch4[2];
	Delay_mS(5);
	if(TX_8M)
...
{//采样、存储、更新循环指针和循环单元
	...
	switch(para.info.te_mode)
	{
		...
		case 6:{yy=3;ReadAD_MODE6();break;}  //Sample_save()---708ms---20140514	
...
void AD_10BITNew(void)			//4ms,内部单片机AD单次采集AN0
{
	//uint b=0;
	ADCON1=0X3A;			//RA0为单片机内部AD，其余IO复用口全部设为IO口
	ADCON0=0X00;			//选择A/D 输入通道AN0
	ADCON2=0X21;            //数据左对齐21，选择A/D 采集时间8TAD,选择A/D 转换时钟001 = FOSC/8,101 = FOSC/16
	ADCON0bits.ADON=1;		//使能AD转换模块
	Delay_mS(4);            //2
	if(TX_8M)
		Delay_mS(4);
...
void  PressTestStart(void)		//140ms
{
	WriteAD7799(0X08);//指定模式寄存器
	WriteAD7799(0X00);//单次转换模式0X20
	WriteAD7799(0X0A); //0X07转换频率33.3Hz,T_settle=60mS	
...
void PressTestGetRslt(void)
{
	...
	g_testRsltTempL = value.ch4[0];
	g_testRsltTempH = value.ch4[1];
}


void TempTestStart(void)		//80ms
{
	WriteAD7799(0X08);//指定模式寄存器
	WriteAD7799(0X00);//单次转换模式0X20
	WriteAD7799(0X07);//0X07转换频率33.3Hz,T_settle=60mS
...
void TempTestGetRslt(void)
{
	...
	g_testRsltTempL  = value.ch4[1];
	g_testRsltTempH  = value.ch4[2];
}

void TestVol(void)			//4ms,内部单片机AD单次采集AN0
{
	//uint b=0;
	ADCON1=0X3A;			//RA0为单片机内部AD，其余IO复用口全部设为IO口
	ADCON0=0X00;			//选择A/D 输入通道AN0
	...
	g_testRsltTempL = AD_value.ch2[0];
	g_testRsltTempH = AD_value.ch2[1];
}
...
void VoltTestStart(void)
{
	ADCON1=0X3A;			//RA0为单片机内部AD，其余IO复用口全部设为IO口
	ADCON0=0X00;			//选择A/D 输入通道AN0
	//ADCON2=0XA1;          //数据右对齐，选择A/D 采集时间8TAD,选择A/D 转换时钟001 = FOSC/8,101 = FOSC/16.
	ADCON2=0X21;            //数据左对齐，选择A/D 采集时间8TAD,选择A/D 转换时钟001 = FOSC/8,101 = FOSC/16
	ADCON0bits.ADON=1;		//使能AD转换模块
...
void VoltTestGetRslt(void)
{
	...
	g_testRsltTempL = AD_value.ch2[1];
	//g_testRsltTempH = AD_value.ch2[1];
}
