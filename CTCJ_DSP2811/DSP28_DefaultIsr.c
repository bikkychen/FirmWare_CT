   ...
   if(ScibRx_Ready() == 1)
	{
		...
		if(RxCn==9)
		{
		  if(SciCom==0x88)// Ð´ÏµÊý±í£¬ÕâÊÇ³¤°ü£¬ÓÚÊÇÐèÒª¶àÊÕÒ»Ð©×Ö½Ú
		  {
		    RxFlag=0;
		  }
		  else
...
if(RxFlag>0)
		...
		case 0x30:	//ÎÂ¶È²ÉÑù//
		 ...
		 case 0xa0:
		 {
			if(SciCom == 0xa0)//ÏÂ·¢Ñ¹Á¦±ê¶¨ÏµÊý
		     ...
		     else if(SciCom == 0xa1)//ÉÏ´«Ñ¹Á¦±ê¶¨ÏµÊý¿ªÊ¼
			 {
			   Col=0;
			 }
			 else if(SciCom == 0xa2)//ÉÏ´«Ñ¹Á¦ÎÂ¶È±ê¶¨Êý¾Ý
			 {
			 ...
			 else if(SciCom == 0xa3)//Çå¿ÕÑ¹Á¦ÎÂ¶È±ê¶¨Êý¾Ý
			 {
   ...
   // if(ScibRegs.SCIRXST.bit.RXRDY == 1)
    ...
    PieCtrl.PIEACK.all |= BIT9;   //ÏìÓ¦ÖÐ¶Ï
	EINT;   //¿ª×ÜÖÐ¶Ï
}
