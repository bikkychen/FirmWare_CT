#include "define.h"
/********************************
        全局变量定义   20140606 校验和0xaff0      V2.0  FOR CYLJ系列压力计
          与 DDI 4芯接口兼容加密后校验和0x0333    20140926--------天石能源科技
*********************************/   
...
{	
	...
	LATB=0;					//B口输出不锁存
	TRISB=0XF1;				//B口B1,B2,B3为输出，其余为输入
	PORTB=0X02;				//B口置初值
...
{//关电前将与AVDD有关的口设为输出0	
	TRISB=0XF0;				//与FLASH有关的4个口线输出0
	PORTB=0X00;				//B口清零			
	TRISC=0X3E;				//1入0出RX   TX                      T1OSI T1OSO
							//      0    0    1     1    1    1    1    0							
	PORTC&=0X03;			//C0、C1值不变，[C2,C7]为0
	...
	TRISD=0X00;
	PORTD=0X30;             //SHDN输出1，关闭AVDD，STA输出1	
}
