 /**************************************************
CTZK
//20161110 增加了流量采样和全部采样的CRC16校验
//20161113 改进了流量数据中时间和相位由无符号短整型转为浮点数时的转换方法，以防上位机软件出现相位超大值
//20161114 流量采样的时间和相位改为整数上传
**************************************************/
#include <iom128v.h>								   	
#include <macros.h>
#include <stdio.h>

#define  BB     0x33        //固件版本号
...
//最好不关刹车，这样电机堵转停止后不会有1.6秒的反向电压，电机不会喘气
...
union MotorStatus
{
 ...
 { 
   ...
   unsigned int DIR:1;//bit1,电机运行方向，0：收臂-调小，1：张臂-调大
...
{   
   ...
   if(PWM1_PIN==1)//电机运行时才实时检测电流，否则电流保存最后一次的测试值
   {
    ...
    if(Motor1Status.s.ThisCurrent>Motor1Thr)//过流关电机
    {
	 MOTORSTOP//关电机
     Motor1Status.s.OverCurrent=1;//过流状态置1，表示电机上次停止原因为过流停
	}
	else if(Motor1Status.s.ThisCurrent<20)//断路关电机，电流小于26mA认为是断路
	{
	 MOTORSTOP//关电机
   ...
   if(PWM2_PIN==1)//电机运行时才实时检测电流，否则电流保存最后一次的测试值
   {
    ...
    if(Motor2Status.s.ThisCurrent>Motor2Thr)//过流关电机
    {
	 MOTORSTOP//关电机
     Motor2Status.s.OverCurrent=1;//过流状态置1，表示电机上次停止原因为过流停
	} 
	else if(Motor2Status.s.ThisCurrent<20) //断路关电机，电流小于26mA认为是断路
	{
	 MOTORSTOP//关电机
...
{ 
  ...
  if(Motor1Thr<77) {Motor1Thr=77;}//堵转电流最小为100mA
  if(Motor1Thr>154) {Motor1Thr=154;}//堵转电流最大为200mA
  if(Motor2Thr<77) {Motor2Thr=77;}//堵转电流最小为100mA
  if(Motor2Thr>154) {Motor2Thr=154;}//堵转电流最大为200mA
